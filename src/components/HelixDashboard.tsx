...
export type HelixDashboardProps = {
  customers: Customer[];
  fileName: string;
  shared?: boolean;
  onReset?: () => void;
  ...
  return (
    <main className="min-h-screen bloom">
      ...
      <div className="mx-auto max-w-7xl px-8 py-10 space-y-8">
        <div className="no-print flex items-end justify-between flex-wrap gap-4">
          <div>
            <p className="text-slate text-xs">
              {shared ? "Shared · Read-only dashboard" : "Internal · Customer success operator view"}
            </p>
            ...
            <p className="text-slate text-sm mt-2 tabular">
              Source: <span className="text-ink font-medium">{fileName || "—"}</span> · {customers.length} accounts
            </p>
            {shareError && (
              <p className="text-red-600 text-xs mt-2">Share failed: {shareError}</p>
            )}
          </div>
          {!shared && onReset && (
            <button
        ...
        {tab === "accounts" && <AccountsTab accounts={customers} />}
        {tab === "renewals" && <RenewalsTab accounts={customers} />}
        {tab === "health" && <HealthTab accounts={customers} />}

        <footer className="pt-8 pb-4 border-t border-parchment flex items-center justify-between text-slate text-xs no-print">
          <span>
            Helix · {shared ? `Shared view · ${fileName || "Untitled"}` : "Customer success operator view"}
            {shared && sharedAt && (
              <span className="ml-2 text-slate/70 normal-case tracking-normal">
                · Shared on {fmtSharedDate(sharedAt)}
              </span>
            )}
          </span>
          <span className="tabular">{customers.length} accounts loaded</span>
        </footer>
      </div>
    </main>
  );
...
function fmtSharedDate(iso: string): string {
  try {
      ...
      <div className="mx-auto max-w-7xl px-8 py-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-baseline gap-2">
            ...
            {shared && (
              <span
                className="ml-3 inline-flex items-center px-2 py-0.5 text-[10px] uppercase tracking-[0.22em] border border-amber bg-amber/20 text-ember rounded"
                title="This is a read-only shared view"
              >
                Shared View
              </span>
            )}
        ...
        <nav className="hidden md:flex items-center gap-6 text-xs uppercase tracking-[0.18em]">
          ...
          {!shared && onShare && (
            <button
              type="button"
              disabled={!hasData || shareBusy}
              onClick={onShare}
              className={`ml-2 inline-flex items-center gap-1 border border-teal text-teal px-3 py-1.5 text-[11px] uppercase tracking-[0.2em] rounded-organic hover:bg-teal hover:text-cream transition ${(!hasData || shareBusy) ? "opacity-40 cursor-not-allowed hover:bg-transparent hover:text-teal" : ""}`}
              title={hasData ? "Create a shareable link" : "Upload a CSV to enable sharing"}
            >
              {shareBusy ? "Uploading…" : "Share ↗"}
            </button>
          )}
        </nav>
      </div>
